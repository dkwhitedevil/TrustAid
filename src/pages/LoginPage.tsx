...
const LoginPage: React.FC = () => {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      const { error } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: { redirectTo: `${window.location.origin}/oauth-callback?signin=1` }
      });
      if (error) throw error;
    } catch (error: any) {
      setErrorMsg(error.message || 'Google sign-in failed');
  ...
  const handleSaveRole = async () => {
    ...
    try {
      ...
      // Get email and name from user metadata
    ...
    <div className="min-h-screen bg-gray-900 text-white flex items-center justify-center py-16 px-4">
      ...
      <div className="relative z-10 max-w-md w-full space-y-12">
        <div className="text-center">
          ...
          <p className="text-gray-300 text-lg">Join the future of transparent charitable giving</p>
        </div>
        ...
        <div className="bg-gray-800/50 backdrop-blur-xl rounded-3xl p-10 border border-gray-700 shadow-2xl">
          ...
          {userRole && (
            <div className="mb-6">
              ...
              <div className={`p-4 rounded-xl bg-gradient-to-r ${roles.find(r => r.id === userRole)?.bgGradient}`}>
                <div className="flex items-center space-x-3">
                  <div className={`p-2 rounded-lg bg-gradient-to-r ${roles.find(r => r.id === userRole)?.gradient}`}>
                    {React.createElement(roles.find(r => r.id === userRole)?.icon!)}
                  </div>
                  <div>
                    <p className="font-semibold text-white">{roles.find(r => r.id === userRole)?.name}</p>
                    <p className="text-sm text-gray-300">{roles.find(r => r.id === userRole)?.description}</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <button
            onClick={handleGoogleSignIn}
            disabled={loading}
            className="w-full py-3 bg-gradient-to-r from-blue-500 to-purple-500 text-white rounded-xl font-medium hover:opacity-90 transition"
          >
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center">
          <div className="bg-white text-gray-900 rounded-2xl p-8 max-w-sm w-full shadow-2xl">
            ...
            <div className="space-y-4 mb-6">
              {roles.map(roleOption => (
                <label key={roleOption.id} className="flex items-center gap-3 cursor-pointer">
                  <input
                    type="radio"
                    name="role"
                    value={roleOption.id}
                  ...
                  />
                  <span className="font-semibold">{roleOption.name}</span>
                </label>
              ))}
            </div>
            <button
              className="w-full bg-indigo-600 text-white py-2 rounded-xl font-semibold disabled:opacity-50"
